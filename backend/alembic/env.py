...
The database URL is taken from vaultstream.config so credentials live in exactly
one place (backend/.env). Override with ALEMBIC_DATABASE_URL when migrating a
different database, e.g. the test database.
"""
